...
<div id="tutorcourse">
  <div style="padding-left:20px;background-color: rgb(242,246,235);">
    ...
    <form action="saveCourseForm" method="post" name="courseform" id="courseform">
		...
		<c:forEach var="oralclass" items="${requestScope.oralclasslist}" varStatus="rowCounter">
			<option value="${oralclass['value']}"  ${requestScope.courseform['lookupclassid'] eq oralclass['value'] ? "selected" : "" } >
                    		${oralclass['description']}
			</option>
  		</c:forEach>
		</select>
